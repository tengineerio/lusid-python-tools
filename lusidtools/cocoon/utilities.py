...
def create_scope_id(time_generator=None):
    ...
    if not isinstance(test, int) and not isinstance(test, float):
        raise ValueError(
            f"The provided response time time_generator.time() is not an int it is a {type(test)}"
        )
...
def default_fx_forward_model(df: pd.DataFrame, FX_code: str, func_p, func_l,
                             mapping: dict) -> (pd.DataFrame, dict):
    """
    Function that takes 2 rows representing a single forward and merge them into a single transaction

    :param df:  DataFrame containing transactions data
    :param T_ID:  Column name of transactions IDs
    :param transaction_type:  Column name of transaction types
    :param FX_code:  The transaction type that identifies a forward
    :param func_p:  function that evaluates to true for where the dataframe row is a buy (incoming) side
    :param func_l: function that evaluates to true for where the dataframe row is a sell (outgoing) side
    :param mapping_cash_txn: mapping for FX transactions
    :return:
    """
    ...
    base_df = fwds_df[func_p]
    fgn_df = fwds_df[func_l]
    ...
    fwds_txn_df = pd.merge(base_df, fgn_df, how="outer", on=[T_ID, t_type],
                           suffixes=[buy_suffix, sell_suffix])

    mapping_cash_txn = remap_after_merge(copy.deepcopy(mapping), buy_suffix=buy_suffix, sell_suffix=sell_suffix)

    return fwds_txn_df, mapping_cash_txn


def remap_after_merge(mapping: dict, buy_suffix: str, sell_suffix: str, file_type="transactions") -> dict:
    ...
    for key in mapping[file_type]["required"].keys():
        if key in incoming_b:
            update_dict_value(mapping, key, mapping[file_type]["required"][key]+buy_suffix, [file_type])
        elif key in outgoing_s:
            update_dict_value(mapping, key, mapping[file_type]["required"][key] + sell_suffix, [file_type])
    return mapping
